...
- 外部成立性admissionでfail closedしたtaskはGLM側の再実行で解決せず、親Codexがtask fileの宣言・evidenceを修復して同じtaskを再開する。拒否時点のtask status・resume checkpoint・pending decisionは保持されるため、decision待ちは同じdecision本文を、rate limit・provider停止・--stop停止中は同じresume actionを再送してよい。`status: poc`/`observation`taskの完了は親Go/No-Go待ち(`NEEDS_SOL_DECISION`)として返るため、Go判断は宣言を`status: implementation`へ書き換えてから`glm-parent-action decision`で渡す。
...
- prepare直後のstaging fileは再読しない。fixではproduction prepare contractが作る既知のtoken binding headerと`__GLM_PARENT_ACTION_PAYLOAD__`だけを前提に、そのplaceholderだけをsemantic payloadへ置換する。decisionではmachine生成templateの`__GLM_PARENT_ACTION_DECISION__`と`__GLM_EXECUTION_UNIT__`を必ず置換し、`MILESTONES_JSON: {"milestones":[]}`を保持する。execution unitは親Codexがsemantic判断として`single`または`milestones`を明示し、`single`では空配列のまま、`milestones`では同じ自然停止境界で残作業を2〜8個のbounded scope/acceptanceへ置換する。token量・file数・generic classifier・追加model callだけでこの選択を決めない。未置換template、legacy free-text decision、`single`とmilestone定義の混在はmachine側がfail closedする。headerを保持し、patch失敗時は実actionを呼ばない。`cat`・`sed`・heredoc・shell redirect・Python等のread/write代替へ切り替えず、staging filenameを推測しない。
- `milestones`を選んだdecisionは同じdecision action内で既存execution-milestone state/revision contractへ接続し、新しいmilestone authorityや別review ceremonyを作らない。既存pending milestoneがあるdecisionではそのauthorityを継続し、`single`でbypassしない。
