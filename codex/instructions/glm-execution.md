...
- Desktop/tool transport境界の長時間接続だけはrepository外である。主model呼出または`glm-parent-action wait`のcode-mode cellでは外側`// @exec: {"yield_time_ms":21600000,"max_output_tokens":4096}`と`background_terminal_max_timeout=21600000`を使い、内側`tools.exec_command`がrunning sessionを返す場合の`tools.write_stdin`も同じ長時間yieldを使う。この指定はlifecycle判断ではない。
