...
- worker/reviewer packetのmachine protocolはcompact 1行JSON。status別field/schema・targets sentinel・size等のstructural validityは`control:packet-schema-result`がfail closedで強制する。親はstructurally valid packetの意味・risk・判断を評価する。
...
- 修正が必要ならCodex自身で編集せず、修正方針本文を`~/.codex/instructions/glm-execution.md`のstdin mode（`--fix-stdin <payload-bytes>`）で同じworker sessionへ差し戻す。再reviewのrole/session/read-only capability分離は`control:reviewer-session-capability-separation`が強制する。Sol自身が現diffの残存部分を受理し、fixが撤回・縮小だけなら`--accepted-scope current-diff`を付ける。不確実または新規変更を許すfixでは付けない。
