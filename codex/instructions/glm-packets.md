...
- 親actionが子process完了後のterminal transport/parseでfail closedし、子側stateが遷移済みか不明な場合もraw artifact/event/telemetry探索から始めない。最初の復旧tool call内でread-only `glm-worker --handoff`を取得し、`task_id`・`task_status`・`consistent`・`required_action`・`allowed_actions`・`pending_decision`・`parent_review_open`・`last_material.call_id/call_type/phase/outcome/packet_status`だけを判断用に機械projectionする。terminal semanticsがなお必要な場合だけ`task_id`と`last_material.call_id`をexact locatorとしてtask-scoped telemetry/eventの該当recordから必要fieldを同じtool call内でprojectし、whole file、artifact directory inventory、複数record dump、広い`jq`結果を先にSol-visible stdoutへ出さない。exact recordが得られなければunknown/errorのまま親判断へ戻し、時刻・filename・directory列挙から推測しない。
