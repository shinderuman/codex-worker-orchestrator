...
planをtask commitへ含め、完了task metadataを個別commit成立後の実状態へ同期する運用では、commit前だけでfinal commit identityと次のrepository状態を確定できない。初回commitと同一commitへのamendからなる二段階で、final HEADのPlan / Task metadataを実状態へ一致させる。通常completion evidenceを`IMPLEMENTATION_HISTORY.md`へ複製する工程は持たない。

1. 実装・test・独立review・必要なSol品質gate完了後、planをcommit-ready状態へ更新する。完了taskをcommit済みと先取りせず、初回commit後に必要となる次task状態だけを矛盾なく準備する。
2. 実装とcommit-ready planを初回commitへ含める。
3. 親Codexが直ちにplanを完了済みcommit・次task・実working tree状態へ同期し、完了task fileを削除して、次のACTIVE/NEXT/BLOCKED参照がfinal treeで成立する状態へ更新する。ordinary completionのcommit・validation・install evidenceはGit / CI / bundleを正とし、Historyへ追記しない。
4. 同期済みplan / task metadataだけを初回commitと同じcommitへamendする。
...
- amend失敗時はobsolete HEADのままinstall・次task・handoffへ進まず、同じcommitへのplan / task metadata同期を復旧して再度amendする。追加commitの連鎖でplan同期を先送りしない。
...
- 順序は「実装・commit-ready planの初回commit → 親CodexによるPlan・IMPLEMENTATION_TASKSの完了同期 → 同一commitへのamend → install.shのgate通過と本配置 → 次task・handoff」であり、installをamendより先に行わない。
...
- task path契約は`taskcontract.ValidateActiveTaskPath`が所有し、runtime ACTIVE解決・repository lint・final-HEAD schedule validationは`PlanSchedule`経由で同じ受理集合を使う。`IMPLEMENTATION_TASKS/` prefix・`.md` suffixを要求し、空segment・`.`・`..`・二重slash・backslashを拒否する。番号prefixは要求せずsubdirectoryを許容する。
- schedule構文は`taskcontract.ParsePlanSchedule` / `PlanSchedule`がACTIVE/NEXT/BLOCKED共通で所有する。逆引用符は項目全体を1組で囲む場合だけpath区切りとして扱い、逆引用符なしの直書きは項目全体をpath候補とする。閉じbacktick欠損・前後の余分なtext・複数backtick組はmalformedとしてACTIVE/NEXT/BLOCKEDすべてでfail closedに拒否する。schedule欄のlist記法は`- `bulletとblank行だけを許容し、`*`・`+`・番号付きmarker等のtask-like list行や説明文などの非bullet行も黙って無視せずACTIVE/NEXT/BLOCKEDすべてでfail closedに拒否する。runtime、repository lint、final-HEAD validationはいずれもこのcanonical schedule semanticsを消費する。
...
- gate失敗時はinstall・次task・handoffへ進まず、Plan・IMPLEMENTATION_TASKSを完了同期して同一commitへamendする。追加commitの連鎖やworking treeだけの修正で通過扱いにしない。amend失敗でobsolete HEADが残っている間もgateは拒否し続ける。
