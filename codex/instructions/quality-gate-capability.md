...
- task finalizationの`glm-parent-action finalize-check <form>`は、current `--handoff`に同一form・PASSのcurrent-snapshot validationがあれば、その`working_dir`を内部で検証してfresh gateの実行dirへ使う。caller cwdはそのrouting evidenceがない場合だけ従来どおり使い、completed validation自体をcacheとして再利用しない。
