...
- task finalizationの`glm-parent-action finalize-check <form>`では、fresh validationとcurrent handoff / repository snapshotのbindingを`control:quality-snapshot-binding`が強制し、親はquality criteriaと最終semantic acceptanceを判断する。実行dirはcurrent `--handoff`の`routing_evidence`（同一form・PASS・同一repository・同一HEAD/index・同一implementation worktree digest。parent-managed metadataだけの差分は`parent_metadata_only`として許容し、exact一致は`exact`）の`working_dir`を、repository配下かつ`go.mod`を持つmodule rootであることを機械確認した上でfresh gateへ使う。evidenceが不存在・解決不能・module root不成立などの通常の利用不能ならcaller cwdを同じmodule root検証へ通し、evidenceもcallerも成立しなければ`stage:"routing"`・`reason:"no_module_root_working_directory"`のstructured failureとしてgateを起動せずfail closedする。ただしevidenceのworking_dirがrepository外を指す場合は利用不能ではなくstate/authority不整合として扱い、callerへfallbackせず`stage:"routing"`・`reason:"routing_evidence_outside_repository"`でfail closedする。選択dirと根拠は結果JSONの`routing`へ載せ、completed validation自体をcacheとして再利用しない。
