...
- `NEEDS_SOL_REVIEW`では親がbindingの`sol_question` / `targets`を意味的に確認したうえで、そこを投影する通常経路に`glm-parent-action review-evidence`を使う。追加targetが必要な場合だけgeneric evidence経路を使う。
