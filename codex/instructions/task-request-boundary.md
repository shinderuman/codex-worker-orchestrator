...
- milestoneはtask fileへ追記するsemantic requirementではなくruntime execution authorityである。ACTIVE taskのContract/Must-not/Acceptanceを変更・複製せず、各unitは短い識別子、boundedなscope、そのunitのacceptanceを持ち、必要な場合だけfresh workerを要求する。
- milestone start/revisionのstaging payload、token binding、field/schema validation、合法transition、worker-mode dispatchは`control:parent-action-staging-admission`とcurrent `glm-parent-action` runtimeを正とする。親はmilestoneの意味・scope・acceptanceを決めるが、exact `prepare`/token/placeholder/JSON/実action手順をこのinstructionから再構築しない。
- 実装途中でmaterially大きいと判明した場合はhealthyなin-flight callを止めない。`waiting-decision`、`waiting-sol-review`、rate limit、provider unavailable、recoverable guard、明示stop等の自然な停止境界だけでmilestone revisionを行い、完了済みmilestoneと停止中in-flight milestoneは変更しない。
