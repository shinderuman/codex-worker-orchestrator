...
- `IMPLEMENTATION_RULES.md`の再読境界では`glm-parent-action authority rules` / `plan` / `active`を同一tool turnで並列実行し、3出力の`authority_snapshot_sha256`と`active_task`一致を確認して各本文を1回だけ読む。不一致・失敗は1回だけ再取得し、再び不一致なら停止する。注入済みAGENTSをdisk再読せず、authority path探索・`wc`等のsize probe・条件付きinstructionの先読みをしない。
