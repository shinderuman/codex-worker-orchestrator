...
- decision: #368のimmutable contractをcurrent mainで再監査し、旧14件inventoryは不十分と判定した。current controlは29 logical controlへ整理し、production ownerの有無で`machine-enforced / partial / prose-only / semantic-parent-only / external-unenforceable`を区別する。
- new gaps: 新たに成立した独立high-risk gapは#394（Codex 5h wake transaction）、#395（failure artifact confidentiality）、#396（Sol review evidence before accept）へDO化した。既存owner #362/#369/#370/#371/#372/#390およびcurrent Plan責務を重複実装しない。
- boundary: backup等の任意host破壊操作とtrusted user-turn identityが必要なuser/direct-edit authorityはrepository単独では完全強制不能、改善候補の採否等はsemantic-parent-onlyとして扱い、false machine guaranteeを作らない。
- thinning: #372はmachine ownerが成立したcontrolだけを薄くする。上記未解決partial/prose-onlyを解決前にmachine-enforced扱いしない。削除済みcontinuous-improvement candidate state machineを復活させない。
- reevaluation: `IMPLEMENTATION_TASKS/post-105-codex-efficiency-reevaluation.md`はこのdecision以後のcontrol delta、未処理candidate、guard違反、locator driftだけを再評価し、全履歴inventoryを再実行しない。
