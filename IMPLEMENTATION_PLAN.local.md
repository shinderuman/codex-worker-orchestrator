...
- current implementation: PR #21 hardening campaignはF1-F10およびF9 hard dependency 016まで実装・review・恒久lint・runtime completion済み。F10 / PR #20はSquash Merge待ち。ユーザー指定scopeにより完了後は通常Plan taskを開始しないため、018をcampaign-stop sentinelとしてACTIVE欄に保持し、010以降は未完了NEXTのまま維持する
...
F10 implementation/runtime completionは完了。PR #20 Squash Mergeとcampaign handoff更新後に停止する。通常Plan taskはユーザーの明示的な再開指示まで開始しない。

## 次の親Codex操作

PR #20本文へF10最終証跡を同期し、final completion treeのRepository Lintを確認してReady化・Squash Mergeする。integrationへF10 merge SHAを記録し、PR #21をcampaign COMPLETE / STOPへ更新したら停止する。010以降の通常Plan taskは開始しない。
