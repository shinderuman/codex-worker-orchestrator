...
次のCodex dogfoodより先に入れるpre-dogfood改善のうち、固定prompt overheadのproduction実装は完了したが実機評価はcommentlint dogfood待ち。instruction conflict、containment denial、parent finalizationの3件を先にWeb GPT側で完了し、commentlintを改善後の最初の観測runとして未着手のまま保持する。

## 次の親Codex操作

ACTIVE `IMPLEMENTATION_TASKS/codex-instruction-conflict-reduction.md`を完了し、続く2件のpre-dogfood改善を順に完了する。その後commentlintをCodex + GLM dogfoodとして実行し、直後に`IMPLEMENTATION_TASKS/codex-desktop-prompt-overhead-reduction.md`の残存Acceptanceをbundleから評価する。bundle task diff修正はその後の独立taskとして残す。
