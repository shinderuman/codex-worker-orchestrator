...
次のCodex dogfoodより先に入れるpre-dogfood改善のうち、固定prompt overheadとinstruction conflictのproduction実装は完了したが実機評価はcommentlint dogfood待ち。containment denialとparent finalizationの2件を先にWeb GPT側で完了し、commentlintを改善後の最初の観測runとして未着手のまま保持する。

## 次の親Codex操作

ACTIVE `IMPLEMENTATION_TASKS/glm-containment-denial-explanations.md`を完了し、続いて`IMPLEMENTATION_TASKS/parent-finalization-choreography-reduction.md`を完了する。その後commentlintをCodex + GLM dogfoodとして実行し、直後に固定prompt overheadとinstruction conflictの残存Acceptanceをbundleから評価する。bundle task diff修正はその後の独立taskとして残す。
