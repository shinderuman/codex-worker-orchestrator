...
- current implementation: F7 `prose-semantic-guard-migration`はPR #17のSquash Merge commit `1b4546548a5089c8160f6f53263ea91fb39a821d`としてCOMPLETE。F8 `instruction-conflict-resolution`がACTIVE
...
latest integrationをF8 branchへ履歴を書き換えず同期し、Rules / Plan / ACTIVE F8 taskを再読する。instruction conflict resolutionの現行production authorityとconflict pathを棚卸しし、F8 production実装を進める。
