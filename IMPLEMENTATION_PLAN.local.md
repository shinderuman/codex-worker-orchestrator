...
- current implementation: `unknown-production-surface-risk`がACTIVE。`hardening/agent-guardrails`の最新HEADから専用task branchを作成してF2を開始する
...
F1はintegrationへSquash Merge済み。F2の開始を妨げるdependency / permission waitはない。

## 次の親Codex操作

integration HEADから`unknown-production-surface-risk`専用task branchを作成し、known-safe LOWとunknown production-like surfaceを区別するTrack A / Track B境界を実装・検証する。
