...
`instruction-surface-guard-recovery-state`を完了。ACTIVEへ昇格したTask 020は未着手で、ユーザー指定の停止境界により開始していない。

## 次の親Codex操作

ACTIVE `IMPLEMENTATION_TASKS/020-repo-search-telemetry-eval.md`をrepository authorityとtask本文から再bootstrapし、通常workflowで開始する。Issue #111は今回の回復整合後もprotected instruction変更の明示的parent handoffを持たない具体gapが残るためOPEN維持とする。
