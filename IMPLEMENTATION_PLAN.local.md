...
- current implementation: PR #21 hardening campaignはF1-F10およびF9 hard dependency 016までCOMPLETE。F10はPR #20 `6633569d2e30acd62470de74b3966bd824cef1af`としてintegration済み。ユーザー指定scopeにより通常Plan taskは開始せず、018をcampaign-stop sentinelとしてACTIVE欄に保持し、010以降は未完了NEXTのまま維持する
...
PR #21 hardening campaignのtask実装はCOMPLETE。通常Plan taskはユーザーの明示的な再開指示まで開始しない。campaign closureとしてintegrationからmainへの非Squash mergeとhandoff更新だけが残る。

## 次の親Codex操作

`hardening/agent-guardrails`をcurrent `main`へ非Squash mergeし、main側Planのbranch境界を`main`へ同期する。PR #21をcampaign COMPLETE / STOPへ更新したら停止する。010以降の通常Plan taskは開始しない。
