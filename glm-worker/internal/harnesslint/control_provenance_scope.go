...
func scopedControlProvenanceViolations(root string) ([]Violation, error) {
	applies, err := controlProvenanceApplies(root)
	if err != nil {
		return nil, err
	}
	if !applies {
...
func controlProvenanceApplies(root string) (bool, error) {
	registryPath := filepath.Join(root, filepath.FromSlash(controlProvenanceRegistryPath))
	if _, err := os.Stat(registryPath); err == nil {
		return true, nil
	} else if !os.IsNotExist(err) {
		return false, fmt.Errorf("stat %s: %w", controlProvenanceRegistryPath, err)
	}
