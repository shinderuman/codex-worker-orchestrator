...
// 欠けた場合は失敗する。旧caller-side recipe固定形の残留も拒否する。
func TestDecisionStdinTransportContractWiring(t *testing.T) {
	...
	wires := []string{
		...
		"stdinがTTY/PTYならglm-worker自身が読み取り前にraw/noecho相当",
		"読み取り後に元のterminal stateへ復元する",
		"callerは`stty`・raw mode・echo等のterminal設定を一切行わない",
		"stdinがpipe/fileならglm-workerはtermiosへ触れず宣言byte数だけ読み取る",
