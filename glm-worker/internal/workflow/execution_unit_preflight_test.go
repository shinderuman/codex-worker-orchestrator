...
func TestValidateDecisionExecutionUnitPayloadUsesCurrentMilestoneState(t *testing.T) {
	...
	if _, _, err := executionunit.Preflight(w.config, st, emptyMilestones); err == nil || !strings.Contains(err.Error(), "existing pending milestone plan") {
		t.Fatalf("milestone disposition without definitions or plan was admitted: %v", err)
	}

	single := "EXECUTION_UNIT: single\nMILESTONES_JSON: {\"milestones\":[]}\nDECISION:\ncontinue\n"
	if _, _, err := executionunit.Preflight(w.config, st, single); err != nil {
	...
	if _, _, err := executionunit.Preflight(w.config, st, single); err == nil || !strings.Contains(err.Error(), "cannot bypass pending execution milestones") {
