...
func TestFailedResumeCallRecapturesStopParentStates(t *testing.T) {
	...
	if restored.StopGitSnapshot == nil || restored.StopGitSnapshot.ParentFiles == nil || !state.SameParentFileStates(*restored.StopGitSnapshot.ParentFiles, wantStop) {
		t.Fatalf("復元checkpointの停止時親stateが呼出後時点へ固定されていません: %#v", restored.StopGitSnapshot)
	}
