...
- push禁止、GLM commit禁止、対象task外のcommit禁止、許可のないcommit禁止は維持する。

## Must not

- 「過去にcommitしてきた」ことだけを包括的な将来commit許可へ拡張しない。
- commit語を含まない一般的な継続指示だけを無条件commit許可として扱わない。
- push許可、remote ref更新許可、force操作許可へcommit authorizationを拡張しない。
...
- task scope不一致、別repository、別task、push、force/non-fast-forwardを許可しないnegative境界を維持する。
...
- 親Codexが最終reviewし、通常task lifecycleに従ってcommitする。pushしない。

## Historical invariants

- GLMはcommit/pushしない。
- 親Codexはtask/review-follow-upの必要gate通過後、当該taskに明示されたcommit要件に従ってcommitできる。
- Git remote書込みは別の明示許可を必要とし、local commit許可から推移しない。
...
Task 009完了時のcommit承認false negativeを3回観測。ユーザーが「さっさとやれ」と直接再指示した後にcommit成功。source comment absolute invariant taskの明示最優先を維持し、その完了後に本taskを実施する。
