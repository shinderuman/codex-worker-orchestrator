...
- `post-worker-quality-gate-recovery.md`のSol判断により、post-worker recoveryはlint violationとgate実行errorの区別・失敗本文保持を担当し、environment failureとharness internal errorの型分類は本taskへ集約する
...
- shared validatorはmissing tool・version不一致等のenvironment failureと、validator/harness自身のinternal errorを呼出側が文字列解析なしで機械分類できるtyped resultを返す
...
- error message文字列のprefix・substring一致でenvironment/internalを分類しない
...
- missing tool・version不一致がenvironment failure、validator/harness faultがinternal errorとして異なるstructured classificationを返すfixtureがある
...
quality-surface承認継続機構はmain基点の統合worktreeでreview・validation・accept済み。初回preflight implementation diffとtask/session/checkpointを保持したままACTIVEへ昇格し、専用approve-surface経路で独立reviewへ戻す。
