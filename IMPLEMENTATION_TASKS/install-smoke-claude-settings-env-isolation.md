...
- existing Claude settings location authorityはinstaller/runtimeで共通resolverを使用し、`CLAUDE_CONFIG_DIR` と `CLAUDE_SETTINGS_FILE` が異なるsettings fileを指す場合にfail closedする。本Findingではそのproduction resolver自体は正しく動作している
...
- production `claudesettings.Resolve` のconflicting-location errorとcanonical path authorityは変更しない
...
- installer/runtimeのeffective Claude settings location authorityはexisting production resolverを正とする
