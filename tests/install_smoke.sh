...
grep -Fq '## commit authorization source' "$success_case/codex/instructions/git.md"
grep -Fq '最新メッセージ単体にcommit語がなくても既存task lifecycleを継続し' "$success_case/codex/instructions/git.md"
grep -Fq 'この例外はforce/non-fast-forward、タグpush、列挙外ref、他repositoryへのremote書き込みへ拡張しない' "$success_case/codex/instructions/git.md"
grep -Fq 'commit・Git履歴操作' "$success_case/codex/AGENTS.md"
grep -Fq '最新メッセージ単体のcommit語の有無だけでは判定しない' "$success_case/codex/AGENTS.md"
grep -Fq '列挙refへの通常fast-forwardとして恒久許可した場合だけ例外' "$success_case/codex/AGENTS.md"
