...
- `--watch`は現在taskの受動event log(`events/<task ID>.jsonl`)を保存済み内容のread・tailだけで表示する参照専用command。provider/workerへの問い合わせ・AI呼出・repo lock・state書換を行わない。follow対象taskのauthoritative `task.status`が`active`を離れた時点(`waiting-decision`・`waiting-sol-review`・`complete`・`rate-limited`・`provider-unavailable`)・別taskへの切替時に残eventを表示して`WATCH_EXIT: task=<task ID> status=<status>`行を出力しexit 0する。event log消失時は従来どおり`EVENT_LOG_STATUS: removed`表示のみで終了する。
