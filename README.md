...
`--stats`は通常のworker packetへ混ぜず、完了済みと現在のタスクを集計して次を表示する。machine JSON出力の全map集計fieldは0件でも空object `{}`として出てJSON nullにはならず、`current_task.status`はtask不在時nullである。
