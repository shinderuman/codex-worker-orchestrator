...
- 停止を受けたownerは、Claude CLIを独自process group(PGID=child PID)で起動しているため、まずgroupへTERMを送り、bounded猶予(既定10秒)内に残存するchildだけへKILLへ昇格する。direct childのwait完了とgroup非残存を確認した後に、保存済みcheckpoint・role session・要求正文・snapshot基準をそのまま保持した`interrupted`状態を保存してackする。groupへ残存が観測された場合だけackに診断を添える。
...
- 停止要求元はackとしてmachine JSON 1行を受け取る: `{"result":"interrupted","task_id":"...","task_status":"interrupted","resume_available":true}`。停止より先にinvocationが自然終端していたときは現在のauthoritative statusで`{"result":"terminal"|"exited", ...}`へ応答し、確定済みの成功結果を停止で上書きしない。
...
- 統合(元repoでのbranch merge)でHEADが停止時HEADから前進しても、隔離記録が現在task・repo・停止時HEADと一致し、記録branchが解決可能でそのtipが現在HEADへ通常mergeで統合済みで、隔離worktree側stateの出自記録と対称なとき、停止時dirty保持がbyte同一のままなら`--resume`は保持照合を通過する。squash/cherry-pick統合はbranch tipを照合できないため承認対象外で、隔離worktree側state dirは元task完了まで残す。統合が保持対象のdirty fileと触れたconflictは内容hashの変化として検出されfail closedする。解決は親Codexが`stop-worktree.patch`・`stop-index.patch`(tracked diffのみ)を基準に行い、元checkoutを停止時内容へ復元するか統合結果へ明示的に取り込んだ上で同じ`--resume`を再試行する。
