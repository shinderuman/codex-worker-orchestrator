...
`analysis-index.json`はversion 2から`intervals`で区間を分ける。`task_execution`=stats.started_atからlifecycle最終遷移(complete/interrupted)まで、`parent_finalization`=その直後からtask開始を含む唯一の親turnの`task_complete`まで、`subsequent_requests`=当該turn完了後に開始した親turn一覧(帰属`unattributed-subsequent-request`、task消費へ不加算)、`collection`=従来の採取範囲(archived-atまたはbundle-time終端)。包含規則は境界時刻以下のrecordを前の区間へ帰属させ、各区間のtoken deltaはcumulative counterのanchor差分(`baseline_at`/`end_at`に原本時刻保持)で同一recordを二重計上しない。`parent_token_delta`/`parent_wait_calls`はtask_executionへ固定され、再採取でcollectionが伸びてもterminal taskの確定値は不変。`parent_finalization`とsubsequent turn別のtoken観測はphase別fieldへ分離しGLM tokenと同一加算式へ混在させない。turn対応は`task_started`/`task_complete`とturn_idのみを根拠とし、時刻の近さやuser messageで補完しない。証拠欠損の区間・集計は`unknown`、未終端は`open`(end=null、採取時刻で閉じない)とし推定値を確定値として出さない。`subsequent_requests`の列挙はcollection終端で上限を切り、終端後に開始したturnは列挙せず、終端前開始でも完了が終端後なら`open`・end=nullで終端後のtoken anchorやcompletionを取り込まない。current taskはbundle-time終端の進行で観測が進化し、archive済みtaskはarchived-at終端で固定され再bundle時に後続turnを追加しない。terminal遷移前にowning turnの`task_complete`が来る逆転境界は`parent_finalization`を`unknown`とする。
