...
`analysis-index.json`はversion 3。`task_execution`=started_at〜lifecycle最終遷移、`parent_finalization`=直後〜task開始を含む唯一親turnの`task_complete`、`subsequent_requests`=当該turn完了後開始turn一覧(`unattributed-subsequent-request`・不加算)、`collection`=採取範囲(archived-at/bundle-time)境界以下は前区間へ。token deltaはcumulative counterのanchor差分(`baseline_at`/`end_at`)・二重計上なし。`parent_token_delta`/`parent_wait_calls`はtask_execution固定・再採取不変。`parent_finalization`/subsequent turn別token観測は分離。turn対応は`task_started`/`task_complete`+turn_idのみ。証拠欠損=`unknown`・未終端=`open`(end=null)・推定値なし。`subsequent_requests`は終端で切り、終端後開始は列挙外、終端前開始で完了が終端後は`open`(終端後除外)。current taskは進行、archive済みは固定。terminal遷移前にowning turnの`task_complete`が来る逆転は`parent_finalization`=`unknown`。
