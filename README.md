...
親Codexが実行中のglm-worker taskを安全に停止するときは、手動でのPID特定・`kill`・process-group signal送出を行わず`glm-worker --stop`を使う。

```sh
glm-worker --stop
```

- `--stop`はrepo lockを取得せず、対象repositoryのstateから一意に決まる単一目的local control endpoint(unix domain socket)へ固定要求行(`{"type":"stop","version":1}`)を1回だけ送る。任意のmessage注入・汎用control plane・task queueは持たない。endpoint接続とowner側ackが停止authorityであり、lock fileのPIDは診断値にとどまる。workflow実行modeのglm-workerだけがrepo lock保有中にendpointをlistenし、終了時にsocket fileを除去する。
- 停止を受けたownerは、Claude CLIを独自process group(PGID=child PID)で起動しているため、まずgroupへTERMを送り、bounded猶予(既定10秒)内に残存するchildだけへKILLへ昇格する。direct childのwait完了とgroup非残存を確認した後に、保存済みcheckpoint・role session・要求正文・snapshot基準をそのまま保持した`interrupted`状態を保存してackする。groupへ残存が観測された場合は`result=interrupted_cleanup_residual`として安全停止ackとは分離したtyped結果へ診断を添える。
- 停止された元invocationはstderrへ`kind=interrupted`のstructured error JSONを出してnon-zero exitし、PASS・completeを出さない。genericなworker error経路のcheckpoint・session clearへは流れない。
- 停止要求元はackとしてmachine JSON 1行を受け取る: `{"result":"interrupted","task_id":"...","task_status":"interrupted","resume_available":true}`。group非残存を確認できなかったときは同じfield構成で`{"result":"interrupted_cleanup_residual",...,"cleanup_warning":"..."}`を返し、これはinterrupted状態保存済みだが安全停止authorityではない。停止より先にinvocationが自然終端していたときは現在のauthoritative statusで`{"result":"terminal"|"exited", ...}`へ応答し、確定済みの成功結果を停止で上書きしない。
- endpoint fileがない(現在running owner不在)ときは`kind=stop_endpoint_absent`、fileが残っていても接続・ackが得れないときは`kind=stop_endpoint_stale`としてtyped errorでfail closedする。
- 中断taskのtask statusは`interrupted`となり、`--status`の`task_status`・`resume_available: true`で同じ状態を読める。`rate-limited`・`provider-unavailable`とは停止理由が排他であり、rate limit上限・provider障害とは混同しない。
- 中断したtaskのstash・session・stateは破棄しない。`--reset`なしの新規task投入はfail closedし、`--resume`で同じstage/phase/sessionから再開する。

```json
{"error":{"kind":"interrupted","message":"task interrupted by glm-worker --stop; task is stopped and resumable","detail":{"phase":"...","task_id":"...","repo_root":"...","resume_available":true}}}
```
...
停止保存時に、元checkoutの保持基準をcheckpointとstateへ固定する。checkpointには停止時点のgit 3軸snapshot(HEAD・index・worktree digest)と、親管理metadata(IMPLEMENTATION_RULES.md・IMPLEMENTATION_PLAN.local.md・IMPLEMENTATION_HISTORY.md・IMPLEMENTATION_TASKS/配下)を除外したdirty/untracked file列挙(path・index blob hash・worktree内容hashに加え、mode・file type・conflict stageを含むlossless識別子の組)を保存し、stateには停止時tracked diffのbinary patch(`stop-worktree.patch`・`stop-index.patch`)を残す。untracked file本文はpatchに含まれず、保持はhash検証のみであるため、親Codexが別途保持する原本なしには停止時内容へ復元できない。user interruptionによる`--resume`はmodel呼出へ戻る前にこの基準へ現在の元checkoutを機械照合し、保護対象の保持を確認できなければstate・checkpoint・sessionを一切変更せず`kind=worker_error`でfail closedする。保持基準を持たない旧binary停止時のcheckpointも同じくfail closedであり、migration・推定は行わない。照合が承認する停止期間中の変化は、(1) 非親管理dirty/untrackedのpath集合・内容hashが停止時と完全一致することを前提とした親管理metadataのworking tree編集、(2) 停止時HEADを祖先に含むHEAD前進のうち、隔離記録がないときは親管理metadataだけの変更範囲、`--isolate`の隔離記録があるときは記録の実質検証(元task・元repo・作成HEADの一致、記録branchの解決と現在HEADへの統合済み、統合後のbranch tipから現在HEADまでが親管理metadata更新だけ、隔離側出自記録との対称)を通過した隔離branch統合としての非親管理file変更、である。保持対象の内容変化・消失・mode/file type/conflict stageを含む保持識別子の変化・停止後の新規dirty・停止時HEADを含まないHEAD移動(amend・rebase等)・隔離branch統合後の非親管理commit・隔離記録の読込失敗・隔離記録の実質検証失敗は全てfail closedする。fail closedは保持checkpointを破壊しないため、親Codexが`stop-worktree.patch`等(tracked diff。untrackedは親が保持する原本)で元checkoutを停止時内容へ復元した後に同じ`--resume`で再試行できる。

```json
{"error":{"kind":"worker_error","message":"停止時に保持したdirty/untracked fileが変化しています: uncommitted.txt(内容変化)","detail":{"phase":"...-retention-check"}}}
```


## 停止taskを保持したまま割り込みtaskを実行する (`--isolate`)

`--stop`で停止した元taskの要求正本・session・checkpoint・working treeを保持したまま、割り込みtaskを同じrepositoryで実行するときは`--isolate`を使う。同一checkoutの単一slot stateへ複数taskを重ねず、別checkoutによる分離を提供する。

```sh
glm-worker --stop     # 元taskを安全停止する
glm-worker --isolate  # 停止中の元taskから隔離checkoutを作成する
```

- `--isolate`は現在taskがuser interruptionによる`interrupted`停止にあるときだけ受け付ける。task不在・別status・rate limit/provider障害による停止は`kind=worker_error`でfail closedし、worktreeも隔離記録も作らない。
- 割り込みtask実行checkoutとして停止時HEADから`git worktree add`し、branch `glm-worker/isolation/<isolationID先頭8文字>`を作成する。worktree実体は`$GLM_WORKER_HOME/worktrees/<repo hash先頭12文字>/<isolationID>/`配下へ置き、元repoのgit管理外とする。
- 隔離worktreeのpathはrepo hash計算のslotが異なるため、state dir・repo lock・session・artifact・stop endpointが元taskと完全に分離される。親Codexは隔離worktreeをcwdとして通常どおりglm-workerへ割り込みtaskを投入する。Plan(`IMPLEMENTATION_PLAN.local.md`)を持つrepoでは、割り込みtask fileを隔離側`IMPLEMENTATION_TASKS/`配下へ置いて明示pathだけでcommitし、隔離側Plan ACTIVEを割り込みtask fileへ切り替えて(こちらは未commitのまま)から投入する。Plan切替を統合commitへ含めると元repoのACTIVE参照が壊れる。隔離側stateには出自記録(`isolation.origin.json`: 元repo root・元task ID・branch)が保存され、`--status`の`isolation_origin`で読めるため、元taskと割り込みtask・復帰対象を取り違えない機械確認が両側対称に持てる。
- 元repo側stateには隔離記録(`isolation.json`: worktree・branch・元task ID・停止時HEAD)が保存され、`--status`の`isolation`で読める。記録は現在の隔離先を指す単一pointerで、隔離済みstateへの再`--isolate`はworktree・branch・隔離側出自記録の生存を確認して同じmachine結果を冪等に返す。worktree・branchが既に無いstale記録や破損記録は上書きせず`kind=worker_error`でfail closedする。新規task開始(`--reset`を含む)でtask状態と一緒に消える。
- `--isolate`は元checkoutのworking treeと元taskのstate・checkpoint・sessionへ書き込まない。成功ackはmachine JSON 1行: `{"result":"isolated","isolation_id":"...","worktree":"...","branch":"...","task_id":"...","repo_root":"..."}`。
- 隔離taskのcommitを元taskへ統合する時点とconflict解決の責任は親Codexにある。統合は元taskがworker/auto-fix段階で`interrupted`の間か、`complete`等の終端status後に行う。reviewer段階で中断した元taskのresumeは既存のreview snapshot照合でHEAD移動を許さないため、統合済みHEADからのreview resumeはfail closedする。
- 統合(元repoでのbranch merge)でHEADが停止時HEADから前進しても、隔離記録が現在task・repo・停止時HEADと一致し、記録branchが解決可能でそのtipが現在HEADへ通常mergeで統合済みで、統合後のbranch tipから現在HEADまでが親管理metadata更新だけで、隔離worktree側stateの出自記録と対称なとき、停止時dirty保持がbyte同一のままなら`--resume`は保持照合を通過する。squash/cherry-pick統合はbranch tipを照合できないため承認対象外で、隔離worktree側state dirは元task完了まで残す。統合が保持対象のdirty fileと触れたconflictは内容hashの変化として検出されfail closedする。解決は親Codexが`stop-worktree.patch`・`stop-index.patch`(tracked diffのみ)を基準に行い、元checkoutを停止時内容へ復元するか統合結果へ明示的に取り込んだ上で同じ`--resume`を再試行する。
- 統合完了後のworktree・branchの削除は親Codexが`git worktree remove`で行う。glm-workerはworktreeの寿命を管理しない。
- 隔離の境界は構造的である。glm-worker配下の実行(childのcwd・git操作・state書込み)はworktree側へ閉じ、元checkoutのgit statusは隔離実行中も変化しない。modelがrepo外の絶対pathへ直接書き込むような操作はgit worktreeの外であり、この機構の保護範囲外である。
...
`--status`はmachine JSON 1行を出す。task ID(`task_id`)・task status(`task_status`)・task別artifact保存先(`artifact_dir`)・session(`worker_session`/`reviewer_session`)・判断待ち(`pending_decision`/`parent_review_open`)・rate limit状態(`rate_limited`)・provider-unavailable状態(原因分類・試行数・経過を`provider_unavailable`)・再開可否(`resume_available`)・隔離記録(元repo側は`isolation`、隔離worktree側は`isolation_origin`)に加え、対象repositoryのlock実保持(`repository_lock`: held/free、probe不能時はnull)と`task_liveness`を出す。`task_liveness`は`task_status`が`active`のときだけrunning/staleへ値が出て、非active時・probe不能時はnullへ統一する。観測できない値はnull/omitとし、`unknown`・`none`のようなpresentation文字列へ落とさない。task不在時の`task_status`もnullである。`task_status`の受理集合は`active`・`waiting-decision`・`waiting-sol-review`・`complete`・`rate-limited`・`provider-unavailable`・`interrupted`の7値とnullだけであり、永続state上の未知値・破損値は未観測としてnullへ正規化する。`resume_available`は保存済みcheckpointがrate-limited・provider-unavailable・user interruption(`--stop`停止)のいずれかの再開可能停止にあるときtrueになる。`isolation`・`isolation_origin`は`--isolate`を使ったときだけ出現し、未使用時・読込失敗時はfield自体を出さない。lock保持判定はflock実取得の非破壊probeであり、lock file内のPIDは診断情報(`lock_pid`)としてのみ扱う。GLM workerの生存判定・重複起動待避は対象repoのlockだけを根拠にし、別repoのprocess一覧や`pgrep`は使わない。`active`+`repository_lock: free`はstale候補としてrepo固有の復旧へ導く。
...
`--timeline`は追加AI callなしでtask/call単位のtimelineをmachine JSON 1行で出す。callごとにrole・phase・sessionとsession内call番号・resumed別・model alias・message model(実model)・観測窓(最初と最終eventの時刻・span・event数)・result観測値(duration・API duration・turns・token・cost)・tool種別別の呼出数とID対応付けできた測定済みduration(合計・最大)を出す。対応付け不能なtool durationは`unmeasured`計数、result event未観測のcallは`result.observed=false`として推測せず、tool名を持たないblockは`unknown` tool種別へ集計する。task単位ではtool種別別合計(`tool_totals`)とtelemetry由来のsession aging(`session_aging`)を続けて出す。相対barのようなgraph表示は持たない。`task_status`は`--status`と同じ7値とnullの受理集合へ出し、現在task・明示指定task・stats履歴archiveのいずれかで観測できない値・受理集合外の永続値はnullへ正規化する。event logの破損行は`skipped_events`計数として報告し、event logがない現在taskは`event_log.status=none`として正常終了する。event log・telemetry path構築へ使うtask ID(明示引数・現在task両方)はtask採番のUUID v4生成形式(長さ・hyphen位置・小写hex・version・variant)へ検証し、不正形式(相対path・絶対path・非v4等)はfilesystemへ触れずerrorを返す。明示指定task IDのlog不在・読込失敗もerrorを返す。
`--convergence`は追加AI callなしでreview/fix convergenceをround単位で表示する。各task開始時(worker実行前)と各review round開始境界でGit snapshot 3軸digestと変更対象pathの観測(全内容digest・空白行/行末空白/full-line comment除去後の意味digest)をtask単位round log(`rounds/<task ID>.jsonl`)へ`0600`でbest-effort記録し、追記・観測失敗はwarning・CaptureErrorだけでworkflowを止めない。表示はround log・telemetry・event logだけから組み立て、roundごとにreview番号・auto-fix回数・生成worker phase・snapshot digest・前境界に対する差分分類・対応付けたreviewer/worker呼出の呼出数・token・turn・duration・packet結果・risk・risk floor再出力の有無を出す。差分分類は`same-snapshot`(event logで当該worker呼出にtool利用が観測されfile変更toolが無いときは`verification-only`へ細分化)・`comment-format-only`・`doc-change`・`semantic-change`・機械確定不能の`unknown`で、文書fileの追加・変更・削除は内容の意味を機械確定しないためfile種別だけで非意味へ分類せず全て`doc-change`として観測する(AGENTS・instructions・EVAL・仕様等の行動規定文書もここへ出るため省略候補から除外される)。raw string・heredoc・triple quote・行継続を含む内容やshell/yaml等の正規化非対応形式は安全側の`semantic-change`候補として扱う。reviewer/worker呼出の対応付けはround境界の時刻とphaseで行い、seq不連続・reviewer番号不一致でrecord欠落が疑われるroundは分類を`unknown`へ倒して`gap`/`mismatched_reviewer`を出す。task単位では差分分類別のreviewer呼出数・token・duration合計(`summary.by_class`)と未解決issue round・HIGH round件数を出す。round logの破損行は`skipped_rounds`計数として報告し、round logがない現在taskは`rounds_log.status=none`として正常終了する。`task_status`は`--status`と同じ7値とnullの受理集合へ出す。task ID検証は`--timeline`と同じUUID v4生成形式境界を使い、明示指定task IDのlog不在・読込失敗はerrorを返す。分類のための追加model呼出・reviewer呼出の削減・model routing変更は行わない。
`--call-outliers`は追加AI callなしで、対象repositoryの保存済みtelemetry JSONL全file(現在taskとretention内の旧task)からworker呼出の分布とoutlierをmachine JSON 1行で出す参照専用commandである。repo lock・state書換を行わず、読み取り対象recordにprompt/response本文が含まれていても出力へは出さない。`telemetry`は読み取り状態(status: ok・partial・none・dir・file数・`ignored_files`・`unreadable_tasks`)で、dir不在・空(または`.jsonl` file不在)はnone、dirの読取失敗(不在以外)はnoneへ偽装せずprocess境界のinternal error・non-zero exitへ出す。task IDのUUID v4生成形式に合わないfile名は読まずに`ignored_files`へ列挙し、破損などで読めないtask fileがあっても他taskの集計は出して`unreadable_tasks`へ失敗を残す。`report.records`は読み取りrecord総数とcall_type別計数(task呼出・event記録・probe呼出・未知call_type)である。分布(`distributions`)はrole・phase・resumed別groupで、worker呼出のphaseは初回`worker-new`・`worker-explicit-fix`・`worker-auto-fix`(round番号とreport-only再出力変種を丸める)・`worker-decision`の集計categoryへ正規化し、`raw_phases`へraw phase別呼出数を残す。reviewer呼出はraw phase(`reviewer-1`等)のまま集計する。各groupは呼出数・turn数とwall時間のmedian・p95・max・total(観測できなかったturn数は`observed`計数へ区別して分布へ混ぜない)・model alias別呼出数・session数を出す。resumed別の分離でcurrent呼出とresume呼出(5h上限・provider障害・`--stop`後の再開)を区別して表示する。分位は昇順値の線形補間(`percentile_method: "linear"`)で小数2桁へ丸める。`models`はrole・model alias別、`sessions`はsession別の累積呼出・turn・duration(同じsessionへ跨るtask数・resume呼出数・最初と最後の呼出時刻付き)を出す。`tasks`はtask単位のworker呼出増幅で、呼出数・resume呼出数・turn観測済み呼出数(`turns_observed_calls`)・turn合計・wall時間合計・最初のworker呼出(phase・時刻・turn・duration・outcome)・初期呼出後の呼出数・turn合計とduration合計の初期呼出に対する倍率(`turns_x_initial`・`duration_ms_x_initial`、初期呼出がturn観測なしに中断した場合はnull)・phase category別内訳を、turn合計の降順で出す。outlierは`outlier_calls`(同じrole+phase+resumed groupのp95(turn)を超えるworker呼出と、判定に使ったgroup閾値・task ID・call ID)と`outlier_tasks`(turn観測済み呼出を1件以上持つtaskのworker turn合計のp95を超えるtask)で、対象taskを再現可能に特定できる要素を載せる。outlier閾値の算出には母数20以上を要求し(`min_population`、group側の母数はturn観測済み呼出数、task側の母数はturn観測済み呼出を1件以上持つtask数)、未満のgroupは`outlier_eligible: false`として分布値の表示だけを行い、未満のtask集団では`outlier_tasks`を空にする。この母数下限により少数呼出のrepository(8呼出程度のmedia-backupのような運用)で同じ集計を実行しても分布閾値の根拠にはならず、同集計はrepositoryごとのstate dir(repo hash分離)へそのまま適用できる。hard turn cap・session rotation・benchmark追加callは導入せず、この観測はGLM側provider枠・wall timeの記録であってCodex ReductionやQuality Deltaの直接証拠ではない。
`--stats`は通常のworker packetへ混ぜず、完了済みと現在のタスクを集計して次を表示する。machine JSON出力の全map集計fieldは0件でも空object `{}`として出てJSON nullにはならず、`current_task.status`はtask不在時nullで、`--status`の`task_status`と同じ7値とnullの受理集合へ未知永続値を正規化する。
